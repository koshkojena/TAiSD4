...
void findDaysAboveAverage(TemperatureRecord records[], int count);
...
int main()
{
    ...
    do {
        ...
        case 4:
            ...
            break;
        case 5:
            findDaysAboveAverage(records, recordCount);
            system("pause");
            system("cls");
            break;
        case 6:
...
void Menu()
{
    cout << "1. Добавить запись\n"
        ...
        << "5. Определить количество дней, температура которых превышает среднюю температуру за месяц.\n"
...
void findDaysAboveAverage(TemperatureRecord records[], int count)
{
    ...
    for (int i = 0; i < count; i++)
    {
        averageTemperature += records[i].temperature;
    }
    ...
    for (int i = 0; i < count; i++)
    {
        if (records[i].temperature > averageTemperature)
        {
            daysAboveAverage++;
        }
    }

    cout << "Количество дней с температурой выше средней: " << daysAboveAverage << endl;
}
void longestNegativeTemperature(TemperatureRecord records[], int count) 
{
    viewRecords(records, count);
